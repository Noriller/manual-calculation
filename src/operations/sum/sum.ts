...
export function Sum(...numbers: string[]): string {
  ...
  const { digitsQuantity, paddedNumbers, biggestFloat, signs } =
    prepareNumbers(numbers);

  // split the numbers into positive and negative
  const [positiveNumbers, negativeNumbers] = paddedNumbers.reduce(
    (acc, curr, index) => {
  ...
  const secondPadding = [positiveSum, negativeSum].map((s) =>
    s.padStart(newDigitQuantity, '0'),
  );

  const { subtraction: sum, finalSign } = subtractPreparedStrings(
    newDigitQuantity,
    secondPadding,
  );
