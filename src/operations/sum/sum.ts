...
export function Sum(...numbers: string[]): string {
  ...
  let digitsToSum = normalizedNumbers.reduce((acc, curr) => {
    return Math.max(acc, curr.length);
  }, 0);

  const paddedNumbers = normalizedNumbers.map(n => n.padStart(digitsToSum, '0'));

  let sum = '';
  let carry = BigInt(0);

  while (digitsToSum > 0) {
    const digits = paddedNumbers.map(n => Number(n[digitsToSum - 1]));
